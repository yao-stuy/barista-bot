...
func (s *beanjaminCoffee) recoverToObserve(ctx, cancelCtx context.Context) {
	...
	observeStep := Step{PoseName: clawPoseCupObserve, Component: "coffee-claws-middle", Pause: shortPause}
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		if err != nil {
			...
			if errors.Is(err, errNoCupsDetected) && attempt < maxAttempts {
				recoverStep := Step{PoseName: clawPoseCupObserve, Component: "coffee-claws-middle", Pause: shortPause}
